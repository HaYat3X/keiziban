...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $db->prepare('INSERT INTO keizi (message, field, course, Expectation, Communication, member_id) VALUES (?, ?, ?, ?, ?, ?)');
    if (!$stmt) {
        die($db->error);
    }

    $stmt->bind_param('sssssi', $message, $field, $course, $Expectation, $Communication, $id);
...
// データの呼び出し
$stmt = $db->prepare('select p.id, p.member_id, p.message, p.field, p.course, p.Expectation, p.Communication, p.created, m.name, m.picture, m.status, m.course, m.School_year from keizi p, members m where m.id=p.member_id order by id desc limit ?, 5');
...
//　結果を変数におく
$stmt->bind_result($id, $member_id, $message, $field, $course1, $Expectation, $Communication, $created, $name, $picture, $status, $course, $School_year);
